...
.LoadWildData:
    ...
    ; Check if the current map ID is in the pointer table
    call CheckRouteInTable
    jp z, .NoWildData ; Jump to .NoWildData if route not found

    ; Get wild data for the current map
    ...
    jp z, .NoGrassData ; If the encounter rate is 0 (there's no grass data), skip to the water data
...
.NoGrassData:
    ...
    and a ; Perform a bitwise AND on a (checks if the encounter rate is 0)
    jp z, .NoWildData ; If the encounter rate is 0 (there's no water data), jump to .NoWildData
    ld de, wWaterMons ; Load the address of the water encounter table
    ld bc, $14 ; The size of the water encounter table is 20 (0x14)

.CopyData:
    ld a, [hli] ; Load the data from hl into a and increment hl
    ld [de], a ; Store the data in de
    inc de ; Increment de
    dec bc ; Decrement bc
    jp nz, .CopyData ; Repeat until bc reaches zero

.NoWildData:
    ret

CheckRouteInTable:
    ld hl, WildDataPointers    ; Load the base address of the pointer table
    ld a, [wCurMap]            ; Load the current map ID into register b

    ; Iterate over the map IDs in the pointer table
    ld de, FIRST_ROUTE_MAP     ; Load the address of the first route map ID
    ld c, 0                    ; Initialize a counter variable

.CheckNextMap:
    ld a, [de]                 ; Load the map ID from memory
    cp b                       ; Compare with the current map ID
    ret z                      ; Return if equal (route found)
    inc de                     ; Increment to the next map ID
    inc c                      ; Increment the counter variable

    ; Check if all maps have been checked (reached the end of the table)
    cp NUM_MAPS                ; Compare with the total number of maps
    jp nz, .CheckNextMap       ; Jump to CheckNextMap if not all maps have been checked

    xor a                      ; Set A to zero (route not found)
    ret                        ; Return
